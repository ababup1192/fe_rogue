---
name: mapchip-debug
description: "fe_rogue のマップチップ不具合（黒い穴・欠けた壁・繋がらない天井帯）を注釈チケットの world.json から再現・検証して直す標準フロー。「注釈: fe_rogue — frame N」の md が貼られてマップの見た目の修正を求められたとき、「マップチップ直したい」「壁が壊れた」「黒い穴」「チップ(4,1)があるべき」等と言われたとき、material.json の wallMap / autoRules を触るときに、作業前に必ず使う。見た目以外（進行・フロー系のバグ）のチケットは対象外。"
...
- タイルはフロア生成時に確定する。**material 修正は再起動 / 次フロアまで画面に反映されない**。

## フロー

このチェックリストを回答にコピーし、埋めながら進める。**順番を飛ばさない**
（スクショからの目視推定でルールを書くと、実フロアで誤発火して壁を壊す）。

```
- [ ] 1. チケットを読んだ（world.json の cellsInRect / map / materialId を確認）
- [ ] 2. 再現器を校正した（現状チップが world.json と一致することを先に確認）
- [ ] 3. 原因セルを調査した（wallKey / autoRules のどちらが効いているか特定）
- [ ] 4. ルール案を全セル差分で検証した（意図したセル以外が変わっていない）
- [ ] 5. 適用 → テスト → 記録
```
