---
name: visual-dict
description: "画面の絵を作るときの手順と、web/Canvas/Shadertoy の技法をこのエンジンの部品へ翻訳する辞書。画風の決め方（色 3 つ・やらないこと 1 つ）と絵の下限 4 性質のチェックリストを出す。新しいゲーム・新しい画面（View）を書くとき、背景・キャラ・エフェクトを置くとき、「見栄えを良くして」「〜っぽい絵にして」と言われたとき、グロー・vignette・グラデ・残像・パーティクル・ドット絵・完全ループ GIF を作る/移植するときに使う。"
sync: 2026-08-11
---
...
- [ ] 4. 上の 3 行を AGENTS.local.md の「この画面の画風」に書いた

絵の下限（4 性質を、どの部品で満たすか）:
- [ ] 面に階調か質感がある: ____
- [ ] 主役が背景から分離して読める: ____
...
**画風に既定は無い。毎回決め直す。** 絵の下限が要求するのは 4 性質だけで、
...
2. **分解する**: 空や壁の大きな面 = グラデ帯 + ディザ / 遠景 = シルエット /
   モチーフ = 1 画素ずつ意思を持って / 手前 = 霧や枠で締める
3. **面は使い捨てスクリプトで組み立てる**(色ランプ + Bayer や塊ノイズ)。
   手で打つのはモチーフの形だけ
4. **焼いて目視 → 直す、を最低 3 周**。「紙吹雪・機械的な市松・四角い暈」に
   見えたらディザの粒度と掛け場所が悪い。光の帯は 1 本に整理する
5. 検査は `bin/lint-sprite.py`(色数・近すぎる色)と下限 4 性質
...
`Num.*` / `Grid.dimsOfRows` は **0.14.0 から**。自分のゲームが引いている版は
`flix.toml` の `github:ababup1192/flix_game_engine` を見る。

**シェーダーは知らない `kind` が 1 つあるだけで Doc 全体が既定値へ倒れる**
（エラーは出ず、絵だけ別物になる）。書いたら必ず焼いて確かめる。
