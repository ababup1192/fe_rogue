...
description: "Flix プロジェクトのテスト設計指針。何をテストし何をテストしないか、期待値を Doc（JSON 既定値）から導く書き方、1 テスト 1 アサート、Effect ハンドラによるモックを出す。テストを設計するとき、新しい Scene ファイルやモジュールが作られたとき、Game.flix などゲームロジックを編集したとき、テストの品質が下がっていると感じたときに使う。"
...
```
- [ ] 1. テスト対象は「ルール・進行・収支」か「JSON 橋渡し」のどちらか: ____
       （モーション・表示タイミング・座標の写経ならテストを書かない）
- [ ] 2. 期待値を Doc（JSON 既定値）から導いた（数値リテラルを貼っていない）
- [ ] 3. JSON 橋渡しなら 1 Doc につき 3 本以内に収めた
- [ ] 4. 1 テスト 1 アサート。テスト内に分岐もロジックも書いていない
- [ ] 5. パターンマッチを使っていない（Option.map / flatMap + assertEq で書いた）
```
...
1. **ゲームのルール・進行・収支**（状態遷移、境界条件、勝敗や資源の帳尻）
2. **JSON（Doc）とプログラムの橋渡し**（読み込み・fail-open・rows の追随）

**テストしないもの**:

- モーション・表示タイミング・座標の写経（粒の見た目の遷移、歩きコマ、演出のイージング等）。
  演出のリグレッションは golden（bake のバイト比較）と目視の仕事であって、テストの仕事ではない
- ただし **クリック当たりの有無**（この状態では押せない等）はルールなのでテストに残す
- initialState の値の写経など、情報量ゼロのテスト。重複は統合する

## 期待値は Doc（JSON 既定値）から導く

Doc に出した数値をテストで使うときは、**数値リテラルを貼らず、Doc 経由の式で導く**
（例: `50.0` でなく `Prologue.dayLen(d())`）。バランス調整（JSON 既定値の変更）で
テストが壊れないようにする。後述の「区切りの良い数値」は、Doc に由来しない
純ロジックの入力・期待値に適用する。

## JSON 橋渡しのテストは 1 Doc につき最大3本

1. 壊れた JSON → 既定値に落ちる（fail-open）
2. 1 フィールドだけ上書きした JSON → その値が反映される
3. rows（可変長データ）の長さに追随する

同型の重複は書かない。デコーダの網羅的な検証は共通層（JsonCodec）で 1 回だけ行い、
各 Doc では繰り返さない。
...
（Doc に出した値はこの節の対象外 — リテラルを貼らず「期待値は Doc から導く」の節に従う）
...
- [ ] 対象は「ルール・進行・収支」か「JSON 橋渡し」か（モーション・座標の写経になっていないか）
- [ ] Doc に出した値の期待値は Doc 経由の式で導いているか（数値リテラルを貼っていないか）
- [ ] JSON 橋渡しのテストが 1 Doc につき 3 本以内か
