...
##   `make sync` … engine_core / render_core / engine / engine_ecs を build-pkg し、それぞれを
...
##                    engine_ecs/lib/github/.../0.1.0/      → 6 階層上 (../ x6)  (ide と同じ深さ)
...
# engine_ecs は engine に依存する再利用 ECS lib。examples が利用する。
ENGINE_ECS_DIR       := engine_ecs
ENGINE_ECS_FPKG_SRC  := $(ENGINE_ECS_DIR)/artifact/engine_ecs.fpkg
ENGINE_ECS_TOML_SRC  := $(ENGINE_ECS_DIR)/flix.toml
ENGINE_ECS_SUBPATH   := lib/github/ababup1192/flix_engine_ecs/0.1.0
ENGINE_ECS_FPKG_NAME := flix_engine_ecs-0.1.0.fpkg
ENGINE_ECS_TOML_NAME := flix_engine_ecs-0.1.0.toml
...
.PHONY: help sync sync-engine-core sync-render-core sync-engine sync-engine-ecs clean-locks clean-example-builds

help:
	@echo "Targets:"
	@echo "  make sync                 engine_core / render_core / engine / engine_ecs を build-pkg し、各依存先に配布"
	@echo "  make sync-engine-core     engine_core だけ build-pkg & 配布 (render_core / engine / engine_ecs / ide / examples へ)"
	@echo "  make sync-render-core     render_core だけ build-pkg & 配布 (engine / engine_ecs / ide / examples へ)"
	@echo "  make sync-engine          engine だけ build-pkg & 配布 (engine_ecs / ide / examples へ)"
	@echo "  make sync-engine-ecs      engine_ecs だけ build-pkg & 配布 (examples へ)"
...
sync: clean-locks sync-engine-core sync-render-core sync-engine sync-engine-ecs
...
sync-engine-core:
	...
	@for dir in $(RENDER_CORE_DIR)/ $(ENGINE_DIR)/ $(ENGINE_ECS_DIR)/ ide/ examples/*/; do \
		toml="$$dir/flix.toml"; \
		if [ -f "$$toml" ] && grep -qE "ababup1192/(flix_engine_core|flix_game_engine|flix_render_core|flix_engine_ecs)" "$$toml"; then \
...
sync-render-core:
	...
	@for dir in $(ENGINE_DIR)/ $(ENGINE_ECS_DIR)/ ide/ examples/*/; do \
		toml="$$dir/flix.toml"; \
		if [ -f "$$toml" ] && grep -qE "ababup1192/(flix_render_core|flix_game_engine|flix_engine_ecs)" "$$toml"; then \
...
# engine_ecs は engine を依存にする再利用 ECS lib。examples に配布する。
# engine に依存するので sync チェーンでは sync-engine の後に置く（lib に engine fpkg が要る）。
sync-engine-ecs:
	cd $(ENGINE_ECS_DIR) && $(FLIX) build-pkg
	@for dir in examples/*/; do \
		toml="$$dir/flix.toml"; \
		if [ -f "$$toml" ] && grep -q "ababup1192/flix_engine_ecs" "$$toml"; then \
			target="$${dir}$(ENGINE_ECS_SUBPATH)"; \
			mkdir -p "$$target"; \
			depth=$$(printf '%s' "$$dir" | tr -cd '/' | wc -c | tr -d ' '); \
			upcnt=$$((depth + $(SUBPATH_DEPTH))); \
			rel=$$(printf '../%.0s' $$(seq 1 $$upcnt)); \
			ln -sfn "$${rel}$(ENGINE_ECS_FPKG_SRC)" "$$target/$(ENGINE_ECS_FPKG_NAME)"; \
			ln -sfn "$${rel}$(ENGINE_ECS_TOML_SRC)" "$$target/$(ENGINE_ECS_TOML_NAME)"; \
			echo "[sync-engine-ecs] $$target"; \
		fi \
	done
