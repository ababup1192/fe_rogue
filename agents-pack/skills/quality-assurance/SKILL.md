...
- モーション・表示タイミング・座標の写経（粒の見た目の遷移、歩きコマ、演出のイージング等）。
  演出のリグレッションはスナップショット（bake のバイト比較）と目視の仕事であって、テストの仕事ではない
...
**単語は業界の言葉をそのまま使う。** ゲーム業界・ソフトウェア業界で通じる語
（スプライト・レイヤー・バージョン・キャッシュ・クランプ）はカタカナか英語のまま書く。
和語へ言い換えず、造語も作らない。読み手はエンジニア。

**説明は平易に。** 難しい数学用語や、業界語ではない生の英単語のカタカナ化に寄りかからず、
何が起きるのかを普通の言葉で書く。
...
数学用語が必要なときは **言い換えを併記**する（例: 「正規化（長さを 1 にそろえる）」）。
...
- [ ] 説明は平易な言葉か（単語は業界の言葉のままか・和語へ言い換えていないか）
