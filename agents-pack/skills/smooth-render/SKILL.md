...
評価されるので、**面の枚数ではなく面の広さ**が焼く時間に直結する。次の 3 つを先に書く。

| 決める物 | 目安 | なぜ |
|---|---|---|
| 解像度 | 描き出す実寸（例 512×320） | 輪郭のなめらかさと焼き時間が両方ここで決まる |
...
1. ゲームの `make render`（出口は `gallery/`）で焼く。`t` は固定する
2. `python3 bin/img-digest.py <png>` で色数・密度を下見してから目で見る
3. **実寸と 2 倍の両方**で見る。倍にすると縞とエアブラシがすぐ分かる
4. 上の失敗 3 つを名指しで自問する —
   ①どこか 1 つだけ彩度が浮いていないか ②エッジの硬さは 3 種類あるか
   ③空に縞は無いか・ムラは 1 種類だけになっていないか
5. **焼く → 目で見る → 直す、を最低 3 周**。1 周目で下限を満たし、2〜3 周目で上の 3 つを潰す
6. `python3 bin/lint-view.py` は矩形と円だけの絵を弾く検査。通っても絵が良いことにはならない。
   良し悪しを決めるのは人（`/critique-render` で目視の批評を取る）

## 禁止

1. **1px の細い面を積んだ疑似グラデーション** — 焼き時間に直撃する。`Shade.Gradient` で描く
