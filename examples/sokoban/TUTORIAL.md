...
- A game's "every frame" is just **calling a function that returns the list of
  things to draw**, over and over.
- The function you edit, almost always, is that function (`frame`).
- Colors are picked from the DB32 palette **by role name**. Constraints make
  decisions easy.
- Even without images, simple shapes composed with **ratios and stacking order**
  can read as "a wooden crate."

There is still no state and no motion. In the next chapter you will want to
**move** the crate — and only then does the question "where does its position
live?" arise, bringing the first new word with it.
