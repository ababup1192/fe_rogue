...
- **F7 残り sim state→World 完了**: waited/alerted/isDying/attackTargetId/selection/drafts 等を World へ（不変条件の達成）。
  ...
  - **F7 残フラグ（同型横展開）**: `attackTargetId`・`acted`（transient/per-turn）→ `prevPos`（sticky）→ `isDying`（最後・async death gate＝別 spike）。各：field＋Cmd＋applyCmd＋preserve＋検証器＋全書込 dual-write＋reader flip。
