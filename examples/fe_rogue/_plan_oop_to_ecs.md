...
- **F3 移動を System に**（**敵ターンより先＝最安全**。position は既に `Cmd.Move` 権威・decision(`Board.tryMoveWithRange`)純粋・`gatherStep.now` は既に World 読み）: `physicsStep`/`gatherStep` → `stepPlayerMove`。Tween は event 再生へ。
  ...
  - **F3c（次・要 spike/run）**: move System を per-node `physicsProcess`（OOP callback）から**フレームパイプライン**へ移設＝着地タイミングを触る唯一の段。gating 混在（isMoving=Tween 待ち＝view / immobilized=status＝world / walkable・range・enemy＝board）を「view が attempt 可否を gate → System が legal 判定」に層分け。timing-sensitive ゆえ O1 harness spike で per-frame 一致を先に実証してから着手（revert 地獄ゾーン）。
