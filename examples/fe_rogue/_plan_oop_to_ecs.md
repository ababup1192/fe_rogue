...
- **F7 残り sim state→World 完了**: waited/alerted/isDying/attackTargetId/selection/drafts 等を World へ（不変条件の達成）。
  ...
  - **🔍 残フラグのスコープ精緻化**: `acted` は **view 状態（行動済み暗転）で sim 決定が読まない＝World 化不要**（relaxed gate の view 残置）。`isDying` は CounterAttackRules が読むが「hp<=0」（既 World）で代替可の可能性。残る**真の sim flag**＝`attackTargetId`（counter target・anyAttacking は view gate）/`followUpUsed`（followUp 決定）/`waited`（味方ターン flow）＝**全て CombatScene の attack:hit 経由 or 味方ターン**＝敵ターン driver と別クラスタ・counter/followUp の多フレーム複雑性に絡む（F4 を低実現可能性にした view-triggered 結合と同根）。**末尾セッションで突入は revert-zone**。次セッションで腰を据える。
