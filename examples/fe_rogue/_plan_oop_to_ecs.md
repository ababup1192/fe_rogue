...
- **F6 戦闘を System に（read-model 反転がここで顕在化）**: hp/weapon/stats を World 権威化 → `Combat` の純解決（`Combat.estimate`/`resolveStrike` は既に純粋）を `stepCombat(world)->（world,events）` に。`attack:hit` 待ちは `Hit`/`Died` event の再生へ。
  ...
  - **🚧 F6a 残（hp 書込 ~28 サイト）**: 残りは CombatScene の heal/lifesteal（`applyPlayerLifesteal`/`applyPlayerWeaponLifesteal` は **World.Command 未持ち＝emit で cascade**）・StaffCast 15・Player/Enemy の restore/levelUp 7。各サイトに `Cmd.SetHp` dual-write（純 hp は `setHp` funnel 検討・複合は単独 emit）。refreshMirror re-mirror 下では全 invariant ゆえ**段階 route 可**。⚠ restore/floor-change（scene 再構築→hp 復元）は F6b（refreshMirror preserve 化）後に Cmd.SetHp 必須＝全サイト網羅が F6b の前提。**集中した次パスで一括 route**（長セッション末尾で残 28 編集を急がない＝revert 回避）。
  ...
  - **F6b 以降**: refreshMirror が hp を command 由来で preserve（位置と同様）＋mid-frame `[F6 HP DIFF]` 検証器（worldRef hp vs scene hp）→ reader flip（combatView 等）→ scene 派生（syncTreeFromWorld に hp 追加）。各段 run 検証。
