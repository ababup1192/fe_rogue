...
- 直したら**前後を並べて見せる**（`make diff DIR=<game>`。変わった絵だけ `debug/diff/` に
  「左=前・右=後」で焼ける）。ファイル名を並べるだけでは、人は 2 枚を切り替えて差を探す羽目になる。
...
| `/visual-dict` | web の絵の技法（グロー・グラデ・vignette・残像・粒・完全ループGIF）をエンジンの部品に翻訳する対応表。見栄えの実装・移植の前に引く |
