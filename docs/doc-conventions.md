...
実装の天井（タブは横 1 列・フォームは縦 1 列・参照解決と検査は 1 段しか追わない）から、
次を目安にする。
...
| 入れ子の深さ | **2 段まで**（record 1 段、または list の item 1 段） | 形を見直す。3 段目は参照解決も検査も効かない |
...
読む側は「名前 → 値」の Map で受け、**知らない名前は目に見える色（マゼンタ）へ倒す** —
...
- [ ] 大きさの目安に収まっているか（タブ 8 枚・1 タブ 15 個・総数 30 個・深さ 2 段）
