...
## style_lab（画風の見本を焼く実験台）で見えた穴（2026-08-14）

丘の場面を coarse / fine / smooth の 3 本の描き手で焼く中で「エンジンに無くて自前で
...
| 層（奥・主役・手前）が分かれているか | z を手で分けた部位 PNG | lint-composition | 半分（`SilhouetteScope.ZWindow` で焼き分けはできるようになった） |
| 当たり判定が絵と合っているか | hitbox.json と絵は別々 | 無し | **足りない**（絵と当たりを重ねて焼く出力が無い） |
...
- **(c) 深度シルエット** — 割に合わない。深度の絵は「何が描かれたか」は言うが
  「その列の地面の表面はどこか」は言わない。曖昧さは残ったままで、焼く費用だけ増える。
- **(b) 部位 PNG に奥／手前の別を持たせる** — z で既に分かる情報の言い換えにしかならない。
  `SilhouetteScope.ZWindow` で焼き分けられるようになったので、これ以上は要らない。
- **(a) 天面だけを焼く** — **横視点（地面が 1 本の線）でだけ正しい。** そこでは
  「列 x の地面の y」が 1 つに決まるので、天面の絵は本物の表面になる。
  奥へ退く面では「列 x の表面」が 1 つに決まらない（同じ列に手前から奥までの
  立ち位置が全部ある）ので、どんな絵を焼いても答えは出ない。

**結論: 画素ではなく数を出す。** 焼くときに `<名前>.layout.json` を並べて書く案を推す。
...
2. **壊れても退行検知に映らない。** `ShaderGen` を呼ぶのは `LwjglLayer` の 3 行だけで、
   リファレンス画像を焼く headless の経路は `ShaderEval` を通るので**このコードを通らない**。
...
寿命はプロセスと同じ**。色や位置は GLSL にリテラルとして焼き込まれるので、Studio で
