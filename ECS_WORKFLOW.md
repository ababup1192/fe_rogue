...
- [~] 5. **最後に** `*Legacy` 4 関数・トグル 3 個・golden oracle・DIFF assert を撤去
  ...
  - ☐ 5c DIFF/checker 掃除（PLAYERDATA/ENEMYDATA/F2 PHASE の縮退・ATTACKTARGET は恒久 dual-write ゆえ残置） ☐ 5d stale twin write 掃除＋field 削除（P2 融合・store 縦スライス）
...
- **次の一手**: P2 read 側（P2-r1/r2）・P4 (a) 全消化・P5 完了。残る大物は 2 つ: ①**P4 (b) engine ItemList/Cursor の resource 化**（メニュー選択状態を engine プリミティブから引き剥がす。**engine 変更を伴うため事前相談必須**）②**P6（Scene ツリー撤去・直列）** — 手順 1 の hide-then-render 廃止から。手順 4 のセーブ形式変更（EcsCodec 直列化）は**破壊的＝ユーザー承認必須**だが、capture は P2-r2 で全面 World 読みになっており射影先の差し替えだけで済む。P2 の残（write 撤去・field 削除・DIFF 撤去）は P6 手順 5（legacy 撤去）と同時に。どちらへ進むかはユーザーと相談。**P2/P6 完了後の次段階（engine への汎用パーツ抽出 = E1 World helper / E2 UI コンポーネント）はルートの `ENGINE_EXTRACT_WORKFLOW.md` を参照**（2026-07-02 策定・前提ゲート = P2/P6 完了）。
