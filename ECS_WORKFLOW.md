...
>
> ### 現在地（2026-06-30）
> S0✅ S1✅ S2.0-2.5✅(🎉S2 store) S3✅(statuses) S4.1✅(unitView twin) S4.4✅(PlayerData twin) S4.4b✅(EnemyData twin) +proj refactor✅ run検証batch✅(twin parity 実証) **S4.3a🟡(fromBoardQuery flip 済・敵ターン run 検証待ち)** S4.2(低価値=skip) S4.3b⬜(他 unitView 消費者) S5⬜ **S6🟡(field flip 進行中: waited✅ bottleUsed✅ / 残=followUpUsed→alerted→attackTargetId)** S7⬜ S8⬜ S9(調査一部✅) S10⬜。1063緑。
> - **S6 field-flip パターン確立（waited→bottleUsed）**: command-derive/read-model フィールドを「scene 権威→World 権威・scene 派生」へ 1 つずつ flip。手順=① reader を World 優先(`World.xxxOf |> getWithDefault(scene)`)へ ② Cmd emit を全 write サイトに ③ **scene 直書きを撤去**（init record 値は残す）④ **syncTreeFromWorld に derive 追加**（撤去後 scene が World 追従＝re-mirror clobber 防止）⑤ flip 完了で `[XXX DIFF]` 検証器撤去（`xxxMismatches` def は dead code 残置・後でまとめて掃除）。refreshMirror の `Query.indexBy` 行は触らない。
