...
- **次の一手**: **P5 完了**（P5-d/e の run 確認待ち）。次は **P2 再開**（在庫の World 単独化。rings/ringEquipped〔player-only 最小〕の縦スライスから。セーブ capture の World 化と不可分＝§D-P2 保留メモ参照）。P2 が済めば **P6（Scene ツリー撤去・直列）**。P4 残（TopBar/Title の△・(b) engine ItemList/Cursor〔要事前相談〕）は P6 の hide-then-render 撤去前までに片付ける。
