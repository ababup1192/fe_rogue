...
> **次の一手 = 完全 ECS 仕上げ**: (1) `acted`/`isDying` view-flag を component or AnimationPlayer effect 読みに整理（acted は waited と重複の可能性／isDying は async death gate）。(2) UI callback ゲート（plan §159）で残 process（Fog/Minimap/Camera/HUD 等）が `(World,UiState)` 純 render か洗い出し。**到達で完全 ECS（Bevy 流）。**
