...
> **次の一手 = F8 本体（dispatch 移設）**: slice1（前順序=combat `anyAttacking`）+ slice2（後順序=TopBar）で **driver の前後順序依存が両方 World 経由＝順序非依存に**なった。残るは driver の per-node dispatch（`EnemyTurnDriverScene.process`・Game.flix:218 redef）をフレームパイプライン段へ移設＝F3c 同型のフレーム順序変更（pacing 待ちは保持・1フレーム化しない）。**到達で完全 ECS（Bevy 流）。** dispatch 移設は F4 同型の探索→spike→run 検証で着手。
