...
>
> ### 現在地（2026-06-30）
> ## 🏗️ B 本体着手（full B = render-from-World・ユーザー GO 判断・2〜3ヶ月マイルストーン駆動）
> **B 開発環境確立✅**: worktree `~/Desktop/fe_rogue-b-gate`（branch `b-gate-spike`）。bin を main から symlink・`make sync` で engine 改修(B0b)込みのパッケージを注入 → **fe_rogue worktree が flix check green**。以降の B 作業はここで（main の A は不変）。
> **engine 改修(B0b) 確定・synced**: `engine_ecs/Render.flix` に `textTinted`（text 着色）＋ `drawAtlas`（region atlas・effectful `\ GameEngine.Game`）。既存 pure `draw` 不変＝dodge 非破壊。
> **マイルストーン順序**: **m1 sprite パイプライン**（map+ユニットを World→drawAtlas で描画・scene-tree と toggle 並走・camera 無し固定視点）→ m2 camera System(要相談) → m3 HUD/menu 移植(parent-child/y-sort 要相談) → m4 重複②撤廃・統合。
> **m1 設計メモ**: ① ユニット sprite の render データ(texture/hframes/vframes/frame/scale/offset または spriteFrames)は **unit resource 由来（spawn 時静的）**で World は resource を R残置で保持 → 静的 sprite は新規 component 不要で導出可。アニメ frame 送りのみ frame-timer component が要る（dodge の `sprites: Map[EntityId,SpriteComp]` 相当）。② render 呼び出しは `GameEngine.Game.renderCommands(drawables, tileMaps, …)`（dodge `Game.flix:50` 雛形）で scene-tree の `GameEngine.render` と toggle。③ map タイルは World/BoardQuery 由来。
> **m1a 配線実証✅（compile green・worktree）**: `RenderWorld.renderUnits(world, scene)` 起こし（ユニットを World.posOf→grid×26px→solidBox）＋ Game.flix:1101 に toggle（`RenderWorld.useRenderFromWorld()` on で `GameEngine.render(scene)` を `GameEngine.Game.renderCommands(renderUnits(...), Nil, Nil)` に置換・現状 on）。**第1版は replace で真っ暗（タイトルでユニット未配置＋メニューも bypass で操作不能）→ overlay に変更**: engine に `GameEngine.renderArgs(scene)`（drawable 抽出を返す・renderCommands は呼ばない・render 挙動不変）を切り出し、fe_rogue は `renderCommands(sprites ++ renderUnits(world), tileMaps, polygons)` で **scene drawable に World box を 1 回で合成**（renderCommands は毎回 clear ゆえ 2 回呼び不可）。通常ゲームは見えたまま box が重なる。**m1a overlay 実機 OK（ユーザー確認・通常ゲーム＋box 描画）**。
> **m1b 座標パイプライン✅実機確定（2026-06-30）**: `renderUnits` を `\ GameEngine.Game` 化し、World.posOf → `gridToWorld`(mapGlobal=`Scene.globalPositionAt(MapScene.worldPath())` + grid*26+13 中央) → `Camera2D.getViewTransform`/`applyToWorldPos`/`applyToWorldSize` で screen 化。**マゼンタ(味方)/黄(敵)マーカーが各ユニットの grid 位置に一致して乗ることを実機確認**（色は既存の移動/攻撃範囲ハイライト〔青/赤縞〕と衝突したため検証用に分離）。⇒ **World→正しい screen 座標が scene-tree と一致＝位置パイプライン完成**。マーカーが胴体やや下なのは sprite offset 分（実 sprite 描画時に res#sprite#offset を合わせれば一致）。> **m1b sprite✅実機確定（2026-06-30・render-from-World で初のゲーム要素が描画）**: ユニットは静的 `Sprite2D`（atlas: texture"rogues"/hframes7/frame）と判明。手動 region 計算でなく **`GameEngine.Renderable.toDrawables(node, screenPos)` + `applyZoomScale` を `pathToDrawables` 同手順で再利用**（texture/region/offset/flip 全部正しい）し、**位置だけ World.posOf→gridToWorld→camera に差し替え**、検証用に cyan tint で overlay。**実機: cyan のユニットシルエットが各ユニットに完全一致**＝位置=World 権威・見た目=実 sprite（region atlas 経路）が scene-tree と一致。⇒ **最初の要素「ユニット sprite」が render-from-World で正しく描けることを実証**。完全移行は m1c。
> **m1c ユニット描画の World 移行✅（build green・実機: 通常どおり）**: ① cyan tint 除去・実色 ② `RenderWorld.hideUnitSprites(scene)`（不変コピーで unit sprite ノードを `CanvasItem.setVisible(false)`）を `renderArgs` に渡し **scene 側 unit sprite を render から除外** → ユニットは **World 由来のみで描画**（重複②の一角が scene-tree から剥離）③ fog 透け対策: `effectiveVisibleAt` で skip・`effectiveModulateAt`(fog/待機暗転)×`effectiveZIndexAt` を適用＝`pathToDrawables` と同じ見え方。`unitDraw` の入れ子 match は **forM で 1 段に**畳んだ（`posOf`×`getEngineNode`）。**m1c 実機 2 バグ→修正✅（2026-06-30）**: ① 武器アイコン追従遅れ＝World sprite が `posOf`(snap)・武器/HPバーが marker tween(滑らか)で desync → **視覚位置を scene の `globalPositionAt`(tween 値)に**し兄弟ノードと同期（World.posOf は存在 gate に降格）② 待機色が暗すぎ＝`effectiveModulateAt`(node 自身の暗転含む)を `d#color`(これも暗転含む)に掛けて暗転² → **`color = effectiveModulateAt` 単一適用**に。`unitDraw` の入れ子は forM で 1 段。
> **★ m1c が露呈した B の核心知見**: **滑らかな移動(tween)が scene 側にある**ため、位置を World.posOf にすると兄弟ノード(武器/HPバー)と desync する。今は視覚位置を scene 由来に逃がしたが、**真の render-from-World は「tween/描画位置を World component 化」が必須**（= m2 の核心。dodge は連続 physics 位置を World に持つ／fe_rogue は離散 grid+tween ゆえ render-position component が要る）。現状=描画 System を renderUnits に移しユニット sprite を scene auto-render から剥離・存在は World gate。**m2=tween の World component 化（ユーザー選択・本筋）着手**:
> **m2-step1 基盤✅（build green・additive・未配線＝挙動不変）**: World に `renderPos`(連続描画位置 pixel・command-derive)＋`moveAnim`{srcPos,dstPos,elapsed,duration} store／`Cmd.SetRenderPos`・`Cmd.StartMove`／**`stepWorld` が毎 tick moveAnim を進め renderPos を `Vec2.lerp`**（＝tween を World System で回す）／`renderPosOf`・`isAnimating`(isMoving の World 版)。⚠ Flix 落とし穴: 型位置 record は `=`・2段ネスト/複数行不可（`Vec2.Vec2` で 1 段化）・`from`/`to` は予約語（`srcPos`/`dstPos` に改名）。**残 step2(配線・要注意)**: ① spawn(addOnePlayer/Enemy)で `SetRenderPos` seed ② `moveTo` が `Tween.tweenPosition` の代わり/隣で `StartMove` emit（duration を tick 換算）③ `renderUnits` が `renderPosOf` で位置を読む ④ `syncTreeFromWorld` が marker 位置を renderPos に（武器/HPバー追従）⑤ `isMoving`→`isAnimating`(input gate・cursor は別) ⑥ 敵 movement も同様。**m2-step2a 味方配線✅（build green・run 検証待ち）**: `moveTo` が `Cmd.StartMove(Player, startWorld, targetWorld, durTicks)` emit（durTicks=`toSeconds(slide)*60` 換算・標準 slide=20/120s→10tick）・engine tween は残し共存（武器/HPバー追従）。`renderUnits.unitDraw` が位置を `World.renderPosOf |> getWithDefault(globalPositionAt)`（renderPos あれば World tween 由来・無ければ scene fallback＝静止セル）。**実機確認待ち**: 味方移動が滑らか・sprite と武器/HP が同期。ズレたら durTicks 係数調整(fps≠60)。**残**: 敵 movement の StartMove emit・syncTree が marker を renderPos 駆動(武器/HP も World 化)・isMoving→isAnimating。**次=m2**: render-position(tween) を World component 化 → 位置も World 由来に。その後 HP バー/武器アイコンも World render へ → m3 HUD/menu → m4 重複②撤廃。**コミット好機**（worktree b-gate-spike: engine renderArgs + Render textTinted/drawAtlas + RenderWorld m1a/b/c+fix）。**m1b**: box→実 sprite（resource 由来 texture・region は `drawAtlas`）＋ mapGlobal offset＋ map タイル描画。
>

> ## 🔬 B0a ゲート結果（render-from-World 試験・worktree `fe_rogue-b-gate`・spike コンパイル green）
> ActionMenu を render-from-World で描く spike（`BGateActionMenuRender.drawActionMenu`）を実装・型検査 green。**実測**:
> - render 書き換え = **51 LoC**（panel+header+items+cursor の*静止描画のみ*）。現 `ActionMenuScene.flix` 434行 + `scene.json` 99行（GUI 編集可）に対して。
> - **⚠ engine gap 2 件を即発見**: ① `engine_ecs/Render.flix:70` の region atlas が `bug!`（fe_rogue は atlas スプライト主体＝sprite 描画に engine 修正必須）② `Render.text` に**色引数が無い**＝menu の文字色(header accent/disabled グレー/cursor 色)が出せず box highlight で代替するしかない。
> - **spike に含まれない**（＝追加コスト）: input/focus/cursor 移動・auto-sizing（ItemList ウィジェットが提供していた）・GUI 編集可能な scene.json（→ 手書き layout 定数化）。item 構築ロジック(434行)は game logic ゆえ残るが ItemList 駆動部は手書き化。
> - **暫定 lean**: render の静止部は安いが、**(a) 文字色すら engine 拡張前提・(b) GUI scene 編集喪失・(c) ウィジェット手書き** が menu×20・HUD×15・scene.json×55 に乗る。B評価資料の「9〜13週・engine 改修前提」を実物で裏付け。
> - **B0b（engine feasibility）✅ engine 改修 2 件を実装・engine_ecs コンパイル green（ユーザー承認済）**:
>   - **text 着色**: `Render.textTinted` 追加＋`textDrawables` で Drawable.color を tint 上書き（SDF は color=文字色）。menu の disabled/cursor/header 色が出せる。
>   - **region atlas**: `Render.drawAtlas`（effectful）追加＝`getTextureInfo` で texSize を引き UV 計算（uvOffset=regionPos/texSize, uvScale=regionSize/texSize・`Sprite2D.computeUv` 相当）。既存 pure `draw` は不変＝dodge 非破壊。
>   - **⚠ 確定したコスト**: region 対応は render extract が **`\ GameEngine.Game` effectful 化**（dodge の pure renderWorld と非互換）。worktree は dep 解決不可ゆえ検証は main repo に copy→check→restore で実施（engine_ecs 単体 green）。
> - **🔬 ゲート総合判定**: **B は feasible（実証済）だが expensive**。① render 静止部は安い（menu 51行）② 即時 engine gap 2 件は修正済 ③ **残コスト**: ItemList/Panel ウィジェット手書き・input/focus/bubbling 手書き（dodge も明言）・camera/parent-child/y-sort/modulate/CanvasLayer の engine System 新規・**scene.json 55個の GUI 編集喪失**・camera/fog/minimap 移植。**B評価資料の 9〜13週・engine 改修前提を実物で裏付け**。⇒ **go/no-go はユーザー戦略判断**（重複② 撲滅が 2〜3ヶ月に見合うか）。worktree `fe_rogue-b-gate`（branch `b-gate-spike`）が spike+engine 改修を保持。
>
...
> - **⚠ run 検証で [USEDSTAIRS DIFF] 発火→spawn seed gap と判明・修正（2026-06-30）**: World=Some(true)/scene=false が全 player 永続。**根本原因＝store は command-derive（`refreshMirror` は preserve+prune のみ・`Query.indexBy` 再構築は `syncFromScene`〔初期 World 構築〕だけ）なのに floor 遷移は syncFromScene を呼ばず `addOnePlayer`+refreshMirror で増分更新**→ scene usedStairs が floor reset で false に戻っても World が前 floor の stale true を preserve。**修正＝`addOnePlayer` の seed battery に `Cmd.SetUsedStairs(false)` 追加**（他 store は全て seed 済・usedStairs だけ漏れ）。同類予防で `addOneEnemy` にも `Cmd.SetActed(false)` 追加。**教訓: command-derive store の新規昇格時は spawn funnel(addOnePlayer/addOneEnemy)の seed battery に必ず Cmd を 1 本足す**（refreshMirror は再構築しない＝floor 跨ぎで stale 化する）。S3 の snapshot-restore Seed gap と同型。**✅ 再 run 検証で [USEDSTAIRS DIFF] 無音確認済（2026-06-30）**。**回帰テスト追加✅**: `TestPlayerScene.testAddOnePlayerSeedsUsedStairs`（`withTracedWorld` で addOnePlayer 後の World に usedStairs=Some(false) が seed されているか pin・修正前は None で fail）。既存 `testAddOnePlayerSeedsPositionToWorld` と同パターン＝position は守られていたが usedStairs だけ穴だった。1064緑。
> - **S5: acted World store 化✅（2026-06-30・1063緑・additive・enemyAlerted 同型）**: 敵行動済みフラグ（view 灰色＋ターン flow）を `enemyActed` store に昇格。store 一式＋`Cmd.SetActed`＋`actedOf`/`actedMismatches`/`[ACTED DIFF]`＋setActed(:422 emit true・effect に World.Command 追加)/clearActedAll(:448 emit false)。effect cascade は `bindSkip`(GameLogger)/`castDone`(Pacing.State) の 2 wrapper に World.Command 追加で閉じた（共に dispatch 配下）。**reader 据置（view-only ＝ flip 不要・syncTree derive も不要・EnemyTurn.Queue が二重行動防止の権威）**。これで turn-state 三点（waited✅/usedStairs✅/acted✅）が全て World store＝**faction-blind tick 統合の前提が揃った**。twin の「S→W 未昇格1(acted)」も解消可能に。
