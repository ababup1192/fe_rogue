...
- [~] 1. hide-then-render 廃止: そもそもノードを積まない（判定: `addChild` 使用が Game/GameLifecycle 以外ゼロ）
  ...
  - ✅ P6-1h ユニット sprite ノード全廃（最重）。新 store 4 つ: `unitAnim={name,elapsed}`（**frame は保存せず描画時に elapsed から純関数導出**＝`frameFromElapsed` が engine advanceFrame と同値。tick は stepWorld の時間加算だけで resource 不要）/ `unitFlipH` / `unitModulate`（本体・HP バー・武器アイコン 3 読者の単一真実源＝P6-1g の sprite modulate 流用を置換）/ `dyingElapsed`（SetDying で 0 seed）。新 Cmd: `SetAnim`（play 意味論=同名 elapsed 継続・別名 0 リセットを applyCmd が再現）/`SetFlipH`/`SetModulate`。renderUnits は resource（sprite/spriteFrames）+ store から Sprite2D **値**を直組み（アニメ個体は frameTexture/frameRegion、静止個体は hframes/vframes/定数 frame）。deathBlink は AnimationPlayer を**タイミング/完了シグナル専用**に残し（Alpha track の書き先は不在 no-op）、alpha は `EnemyScene.deathBlinkAlpha(dyingElapsed)` で曲線導出。hideUnitSprites 撤去。遷移系（showAllActive/applyFacingFlip 等）が Command を持つ効果 cascade は menu/PhaseTransitions/テスト fixture（withCancelTransitionMocks に捨て handler）まで波及。
- [x] 2. `syncTreeFromWorld` / `refreshMirror` / `syncFromScene` 撤去（World が唯一の真実源になる）
  - 調査済み（agent 棚卸し）: 撤去順は **syncTree の derive を field 単位で退役（gridPos 最後）→ refreshMirror の derive 7 系（ids/team/hidden/敵ring/growth/mapSnapshot/effectRule）を Cmd 化 → refreshMirror 縮退 → 起動 seed（syncFromScene 本番唯一の呼び出しは Game.flix start）**。読み手は (A) overlay 経由=既に World / (B) getAll 直読み=真の twin reader の 2 系統で、flip 対象は (B) のみ。
  - ✅ P6-2a syncTree derive を **gridPos だけに縮退**（hp/waited/isDying/alerted/level/exp の 6 derive を撤去）。分類基準: ①waited/hp/level/exp = scene twin 退役（waited は S6 で scene 直書きなし、hp/level/exp は **ECS 経路が World のみ書く**）→ live (B) reader を World 直読みへ flip（firstActive/firstActiveExcluding/commitAllWaitedAndAdvance/lordReadyToGather）＋ **playerDataKey から hp/waited/level/exp・enemyDataKey から hp を退役**（key coverage テストは「含まれないこと」を == で pin に反転）。②isDying/alerted = writer が scene 併記（startDeathAnimation/driver/restore の dual-write）→ derive だけ撤去し DIFF キーに残す。snapshot/carry 系は P2-r1 の overlay 化済みで無傷。WorldQuery の効果 cascade は TurnFlow→ActionMenu/ItemMenu/LevelUpPanel/StaffCast/CombatScene/TurnEndHold/StairsExit/TurnPhase の 6 波＋テスト fixture（withWorldCommand に WorldQuery を同一 Ref 結線=read-after-write）。
  - ✅ P6-2a-fix 実機 [UNITVIEW DIFF]（hp/combat）対応。原因: P6-2a の hp reader 棚卸しが `#hp` **直読みだけ**数え、`unitView(d)` **経由の hp 読み**を見落とし（検証器が正しく検出）。scene unitView 消費者を精査し、**RosterQuery/PartyQuery overlay 経由は既に World-fresh（変更不要）**、生 scene 読みの 5 箇所だけ flip: ①②反撃分岐 resolvePlayerMainAttack/resolveEnemyMainAttack（直前 ECS ダメージの hp で Kill/Counter を決める＝最重要。unitViewFromWorld + World 未登録時のみ scene fallback）③敵 AI の self view（driver）④武器選択ターゲット列挙（CursorScene→RosterQuery.getAll）⑤射程内生存判定 isReachableEnemyCell（WeaponSelect→RosterQuery.get）。startAttack の EntityScene.unitView は pos/range のみ＝残置。**[UNITVIEW DIFF] ロガーは退役**（hp/combat が World 単独権威になり比較は恒常ノイズ・残る twin field は PLAYERDATA/ENEMYDATA DIFF が引き続き突合）。診断過程で追加した TestDiagUnitView（フロア 1 の敵 5 種 spawn-seed parity）は恒久テストとして残す。
  - ✅ P6-2b gridPos twin 退役＋**syncTreeFromWorld を gameLoop から撤去**（関数はテスト seam として残置）。agent 分類（A=overlay 済/B=現在位置 59 箇所/C=移動前位置 5 箇所/D=DIFF seam/E=無関係）に基づき実施: ①C は構造的に解消 — **applyCmd の Move が旧位置を prevPos store へ自動記録**（両 faction・Cmd 列不変・spawn 初回は記録なし）し、敵/味方の move 再生（facing/prevPos）は World.prevPosOf を読む。旧 SetPrevPos emit（replay 内）と scene pre-move 読みを撤去。player moveTo の facing は「emit **前**の posOf」読み。②B は fetch 点 flip — 新ヘルパー `getWithWorldPos/getAllWithWorldPos`（gridPos だけ World 上書き・他 field は scene 権威のまま＝最小差分）を両 scene に追加し、StaffCast/Combat(startAttack)/driver/fog(holes 3 か所)/移動系/範囲表示/hover/照準 等 ~30 fetch を差し替え。beginTurn の board は BoardSnapshot.fromScene → **World.toBoard**（BoardQuery cascade 回避で WorldQuery 経由）。③playerDataKey/enemyDataKey から posKey を退役（coverage テストは == pin へ・drift テストの drift 源は moveTiles へ）。WorldQuery cascade は 10 波超（fixture 3 種にも同一 Ref/空 World handler を結線）。
  - ✅ P6-2c refreshMirror を **tick+1 のみ**へ縮退（scene 引数も撤去）＋起動 seed を `World.empty()` へ。新 Cmd 8 種: `Spawn/Despawn/ClearUnits`（id 集合・team・prune の command 権威化。Despawn はノード除去サイト〔completeDeath/戦死/引き継ぎ剪定/respawnFromInitial/removeAll〕が emit、ClearUnits は build choke〔Playing 2 経路＋Title/CharacterSelect〕が emit）／`SetHidden`（useStairs の setHidden funnel で dual-write）／`SetEnemyRing`（spawn/復元）／`SetGrowth`（**emitBaseStatsFromScene に折り込み**＝resource 変化サイトで自動 pair・spawn でも seed）／`SetMapSnapshot`（build choke で Some・ダンジョン外で None）／`SetEffectRule`（spawn）。player 側 equippedRing は store 廃止でなく **accessor 読み時導出**（rings/ringEquipped store から）。golden は SetGrowth 1 件追加のみ・旧 prune テストは Despawn テストへ置換。**これで手順 2 完了: frame 境界の scene⇄World 同期は全廃**（syncFromScene/syncTreeFromWorld はテスト seam 残置・refreshMirror は frame カウンタ）。
- [x] 3. `applyPhaseChange` の `Scene.empty()` 再構築を「GamePhase 別の World/resource 初期化」に置換
  - **実質完了（P6-2c で達成・再構築は残滓の正当な組み立てとして存続）**: 本手順の狙いだった「phase 遷移＝World/resource の明示初期化」は build choke の Cmd（ClearUnits/ClearChests/ClearItems/SetMapSnapshot＋spawn seed 群）で達成済み。`Scene.empty()` 再構築自体は、**ノードとして残る正当な要素**（Map タイル源・ユニット marker・HUD/メニュー〔P4(b) 未実施〕・driver・Camera/Audio）の組み立て手段としてそのまま残す（prevKeyState 引き継ぎも gameLoop で維持済み）。完全なノード非依存化は P4(b)（メニュー resource 化）と marker 退役（別 phase）の後にしか意味を持たないため、ここでは「World 側の初期化が scene 再構築に依存しない」ことをもって完了とする。
- [x] 4. セーブを EcsCodec 経由の World serialize に置換 → **ユーザー判断でスキップ（目的達成済み・選択肢 A）**
  - 本手順の目的「セーブが scene tree に依存しない」は達成済み: 駒=P2-r1 の overlay capture（World store 由来）、床アイテム/宝箱/階段=P6-1 の World store capture、地形=生成 Plan。FloorSnapshot.flix に scene 読みゼロ。
  - 形式は id ベース（catalog 参照＋動的値）の現行を維持 — World store の生 dump（EcsCodec）より store レイアウト変更に強く、旧セーブも壊さない。EcsCodec は engine_ecs の汎用 encoder として残置（未結線のまま）。
- [x] 5. **最後に** `*Legacy` 4 関数・トグル 3 個・golden oracle・DIFF assert を撤去
  - 調査済み（agent 棚卸し）: 撤去可能なのは戦闘 legacy 解決チェーン（~25 関数）＋`*Legacy` seam 4 本＋杖の Heal leaf のみ。**杖の apply系（Blowback/Swap/Bind/Stopgap/warp 等）は effectRule DSL 経路から共有される LIVE**（命名が legacy なだけ）。共有ヘルパー consumeEquippedWeapon/forecastFromViews/attackForecast/rollPercent も LIVE。
  - ✅ 5a golden の ECS 単独化: TestGoldenTrace の harness（runAttack/runAttackConst/runEnemyAttack）を live `applyAttackHit`/`applyEnemyAttackHit` に flip。(c) 等価 21 本は「wrapper↔system 整合」の pin としてそのまま生存、(a) 8 本の frozen literal を ECS stream で再凍結（GENERATE-THEN-FREEZE。ECS は eventToCmds 先行＋scene 副作用 co-emit 後行の順序・Dying 二重 emit は冪等・敵 seam は武器耐久 co-emit を出さない）。TestCombatRolls/TestCombatScene の二重 World harness（外 WorldQuery+内 dischargeWorldCommand が P6-2a の同一 Ref 化で shadow）を ONE-Ref withTracedWorld に組み替え、hp/exp 検証を World store 読みへ。
...
- 履歴:
  - 2026-07-02: **移行完了後の残党掃除**（3 agent で全棚卸し → Tier 1/2/3 実施）。①Tier1: 呼び出し元 0 の pub def 9 個（`rngOf`/`actedOf`/`effectRuleKey`/`unitViewFieldDiff`/`explosionFxNextSeqOf`/`playerInRange`/`effectSentence`/`emitNormal`/`expectedWeaponFields`）＋陳腐化コメント（実在しない `useEcsCombat` トグル・削除済み行番号参照）を撤去。②Tier2: `Stairs.isAt`（P6-5 完了で撤去条件成立済みの test-only seam・live gate は `World.stairsPosOf`）を削除し TestStairs を `getPos` ベースへ。**`PlayerScene.getHp` は残置**＝scene→scene 変換（heal/reconcile）の出力を読む正当な test seam で stale twin ではない（要精査の結果 keep）。③Tier3: golden の `*EqualsLegacy` テスト ~20 本・helper `runStaffLegacy*` を改名（`applyStaffEffectLegacy` は削除済みで両側 live orchestrator↔純 sim の整合 pin＝"Legacy" は誤名）＋local `legacyW`→`oracleW`。全 tier 挙動不変・1074 green 維持。**live トグルは 0**（移行は完全終了）。学び: agent の "superseded" 判定は live gameplay read 前提＝**test seam の read 文脈（scene 変換出力 vs stale twin）を個別確認してから消す**。
  - 2026-07-02: 本 doc 作成（Scene vs ECS 全棚卸し → P1-P6 ワークフロー策定）
  - 2026-07-02: P6-5c/5d 完了（手順 5 完結）。学び: ①DIFF 検証器は「移行の足場」と「恒久 invariant guard」を区別して畳む — dual-write が設計として残る field の DIFF は資産、権威が片側に寄った field の DIFF はノイズ。②twin 退役後の残 read を最後にもう一周 grep する価値がある（DSL の Combat.damage(hp=target#hp) 入力は writer 表に隠れて reader 棚卸しから漏れていた）。全 1079 green。
  - 2026-07-02: P6-5a/5b 完了（トグル 3 個・戦闘 legacy 閉包・杖 legacy dispatcher 撤去）。学び: ①golden の legacy oracle 撤去は「(c) 等価テスト＝wrapper↔system 整合 pin に転生・(a) stream pin＝ECS で再凍結」の 2 型で機械的に済む（独立 frozen pin を最初から併記していた設計が効いた）。②大量 def 削除は compiler の Unused definition を whitelist と照合して block 削除する半自動ループが安全で速い。③「legacy 命名 ≠ legacy-only」— DSL/共有経路から呼ばれる live 関数の誤削除が最大リスク（agent 棚卸しで事前に閉包判定してから削る）。全 1081 green。
  - 2026-07-02: P6-2c 完了（refreshMirror 縮退・起動 seed 空化）。学び: prune（scene に居ない id の剪定）は「毎フレーム走る防御」から「**ノード除去サイトが emit する Cmd.Despawn**」へ移すと、除去漏れが「store に残る＝DIFF/挙動」でなく「emit 忘れ＝レビューで見える 1 行」になる。ClearUnits は ClearChests/Items で確立した build-choke 掃除パターンの一般化。全 1082 green。
  - 2026-07-02: P6-2b 実機バグ修正②（「戦闘になると表示位置がおかしい」）。原因: **attack lunge の origin/復帰位置**が `EntityScene.unitView` の scene 側 pos（凍結 twin）から組まれ、突進アニメの Position track がマーカーを古いセルへ動かし・戻していた。前回監査で「pos/range だけだから残置可」と誤判定した箇所＝**「見た目のアンカー（ピクセル座標）を組む pos」こそ最優先で flip すべき**（論理判定より視覚破綻が目立つ）。EntityScene.unitView の pos を World.posOf overlay 化して解消。cellCenter/gridToWorld × scene pos の総ざらいで他は legacy-only と自前 store のみを確認。全 1082 green。
  - 2026-07-02: P6-2b 実機バグ修正（「カーソル通り動かない・斜め移動」）。原因は flip 漏れ 8 箇所 — **手動移動の起点**（stepFor の data#gridPos＝node tag 内の凍結値から Move 先を計算→古い位置+方向で斜めジャンプ）が主犯。他に beginSelection/beginGather の巻き戻し origin（silent miss: `Draft.MovedFrom` と書いて置換不一致）、集合追従の prev、階段退場列の front、**青マス表示の盤面と原点**（BoardSnapshot.fromScene 残置）、markDeaths、武器選択の自位置。教訓: ①一括置換は **置換件数を assert**（silent miss が実バグ直結）②node tag 経由の `data#gridPos`（physicsProcess の Data param）は getAll と別系統の読み口＝棚卸しの盲点。修正は全て「emit 前の World.posOf 読み or getWithWorldPos」。全 1082 green。
  - 2026-07-02: P6-2b 完了（syncTree 本番撤去）。学び: 「移動前位置」を意図する pre-move 読みは reader flip では救えない — **書込側（applyCmd）に旧値の自動記録を足して読み手を World へ寄せる**のが構造解。emit 順・Cmd 列を変えないので golden も無傷。cascade が 2 系統（WorldQuery 追加と BoardQuery 追加）に分岐したときは、既に全域へ伝播済みの effect（WorldQuery）へ寄せると波及が 1 桁減る（World.toBoard 置換）。
  - 2026-07-02: P6-2a-fix。学び 2 点: ①reader 棚卸しは field 直読み（`#hp`）だけでなく **view 関数（unitView 等）経由の間接読み**まで追うこと — 集約 view は twin 退役の盲点になる。②診断は「実カタログ＋実 frame パイプラインのヘッドレス再現」→ダメなら「DIFF ロガーの field 内訳強化＋実機 1 run」の 2 段が効率的（今回は後者で hp(w=13,s=15) が一発で出た）。
  - 2026-07-02: P6-2a syncTree 縮退（derive 6 系撤去）。学び: twin 退役の判定は「**その field の全 writer が scene に併記しているか**」で二分される — 併記あり（isDying/alerted）は derive 撤去だけで DIFF も維持できる。併記なし（hp/level/exp は ECS 経路が World 単独・waited は S6 で scene 直書き撤去済み）は (B) 直読み reader を flip してから derive と DIFF キーを同時に退役する。DIFF key coverage テストは「含む」pin を「含まない」pin（==）へ反転して誤復活の防壁として残す。全 1077 green。
  - 2026-07-02: P6-1h ユニット sprite ノード全廃。設計判断 3 点: ①アニメ frame を store に持たず **elapsed（時間）だけ store 化して frame は描画時導出** — stepWorld が SpriteFrames resource（コマ毎 duration/loop）へ依存せずに済む。②deathBlink は「見た目（alpha 曲線導出）」と「タイミング（AnimationPlayer 完了シグナル→removeAt）」を分離し engine 無改変。③暗転は unitModulate store が本体/バー/アイコンの単一真実源（P6-1g の教訓どおり sprite 流用から置換）。効果 cascade の学び: showAllActive 等の Command 化は同一テキストの効果行が複数箇所にあると置換が衝突する（PhaseTransitions 67 行 vs 284 行）＝行番号で precise に直す。全 1077 green。
