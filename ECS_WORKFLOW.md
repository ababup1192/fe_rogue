...
> **F8 本体 着地（2026-06-28・910緑・run 検証待ち）= driver dispatch を OOP callback→frame-paced System へ移設**: `EnemyTurnDriverScene.process(delta,node,path,scene)` は node/path/delta を使わない pass-through（driver は payload-less stateless）だったので純 `step(scene): Scene` System に昇格。`dispatchDrivers`(per-node redef) から `EnemyTurnDriver` ケースを削除し、gameLoop パイプラインに `|> EnemyTurnDriverScene.step`（`GameEngine.process` の後）を明示段として追加。**敵ターンロジックが Node lifecycle callback でなく gameLoop 駆動の System に＝軸2（control inversion）本体達成。** ⚠ **slice1/2 と違い byte 同一でない**: 旧 driver は tree 巡回の途中（combat の後・TopBar の前）、新 driver は process 全体の後。driver は combat/TopBar を live worldRef（今フレーム emit 込み）から読むので読みは最新＝ロジック分岐不変、唯一の差は driver が起こした phase 変化に view が反応するのが ≤1 フレーム遅れる（演出 ≤16ms シフトのみ）。**run 検証 = バー赤⇄青ワイプ/敵ターン開始終了/攻撃反撃追撃にカクつき・ちらつき・引っかかりがないか＋ DIFF 無音。** fallback = `step` を `GameEngine.process` の前へ（1 行）。
> **次の一手 = F8 仕上げ ＆ 残 driver**: (1) 同パターンで他 driver（StairsExit/Fog/Minimap/ArrowCursor）も System 化を検討（StairsExit は CameraScene.isSettled/Pacing 読みが残る）。(2) `acted`/`isDying` view-flag を component or AnimationPlayer effect 読みに整理。(3) UI callback ゲート（plan §159）で残 process が純 render か洗い出し。**到達で完全 ECS（Bevy 流）。**
