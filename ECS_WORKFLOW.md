...
> ## ★最前線（2026-06-28）= 🎉 OOP→ECS 移行 **ゴール達成**。**詳細・現在地は `examples/fe_rogue/_plan_oop_to_ecs.md`**（authoritative）
> **🎉 軸2（control inversion）達成・F8 完了**: 敵ターン進行（EnemyTurnDriver）・階段退場（StairsExitDriver）の LOGIC 駆動を **OOP node-lifecycle callback から gameLoop 駆動の frame-paced System（`step(scene)`）へ移設**＝当初の真のゴール「scene を LOGIC 駆動から脱却・ロジックを World→World System で走らせる」達成。slice1（attackTarget→World）+slice2（TopBar bool→World）で driver の前後順序依存を World 経由＝順序非依存にしてから dispatch 移設。run 検証クリア（≤1f 演出シフト体感不変）。**UI callback ゲート点検済＝残 callback は view-render と input/event 反応のみ（gap 無し・下記）。**
