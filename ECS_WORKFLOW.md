...
- 2026-07-02 **実機 run OK**（P1 敵 Bind・P2-S1/S2 read-flip・bottleUsed/followUpUsed/prevPos 退役までを実機検証・回帰なし）。以降の重量フェーズ（driver 退役）着手前の足場として固定。
- 2026-07-02 **P2-2a batch1: MinimapScene を World 経由読みへ flip**（3 getAll→PartyQuery/RosterQuery・効果を render helper に thread・dispatch checked_ecast が吸収・`FrameAef.ProcessT` が既に両 query を宣言済＝view-driver flip は smooth・unused `scene` param 削除）。R2 破壊レビュー **97 PASS**（6 攻撃全 refute）・1063 green・check clean。★view scene は per-scene 個性あり（FogScene.applyFog は init-time で World 未 seed ゆえ scene 読み据置＝CursorScene:135 と同型の意図的例外・isVisible/findIdAt/displayName は query 等価 or residual 判断）。2a 残 ~14 scene。
