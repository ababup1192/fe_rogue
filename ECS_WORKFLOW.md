...
>   - **★次の一手 = D2**（menu Query-flip・鮮度非依存 read のみ）。D1 残（`PartyQuery.findIdAt`・`stavesOf/consumablesOf/ringsOf` wrapper・currency fixture テンプレ）は消費する D2/D5 で fold。以降 D3 inventory store → … → D14 bridge 撤去 → D15 DIFF teardown。openQ #2-#8 は各 slice 到達時に判断。
