#!/bin/sh
# make bench の中身。焼いたばかりの gallery/*.png を golden/SHA256SUMS.txt と突き合わせる。
#
...
sums=golden/SHA256SUMS.txt

if [ ! -f "$sums" ]; then
	echo "まだ祝福されていません: $sums がありません。" >&2
	echo "いまの絵を基準にしてよければ make golden を実行してください。" >&2
	exit 1
fi
...
if ! diff -q "$tmp/expected" "$tmp/actual" > /dev/null; then
	echo "bench NG: 絵の顔ぶれが変わりました" >&2
	comm -13 "$tmp/expected" "$tmp/actual" | sed 's/^/  増えた: /' >&2
	comm -23 "$tmp/expected" "$tmp/actual" | sed 's/^/  消えた: /' >&2
	echo "  意図した変更なら make golden で基準を更新してください。" >&2
	exit 1
fi

(cd gallery && shasum -a 256 -c ../"$sums")
echo "bench OK: $(wc -l < "$tmp/expected" | tr -d ' ') 枚すべて基準と一致しました"
