...
WORDS = [
    ...
    ("台帳", None, "一覧表 / レジストリ", "registry", "error"),
    ("暗幕", None, "暗くするオーバーレイ", "darkness overlay", "error"),
    ("器", r"(?<![一-龥])器[はをがにへので]", "入れ物 / コンテナ", "container", "error"),
    ("素通り", None, "何もせず通す / スキップする", "pass through / no-op", "error"),
    ("札", r"(?<![一-龥])札(?![幌])", "ボタン / カード / ラベル",
     "button / card / label", "error"),
    ("正本", None, "正しい元データ / 唯一の出どころ", "source of truth", "error"),
