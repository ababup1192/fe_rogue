...
→ **寄せる前に「消すか」を先に決める。** 死んだコードを整備するのは、重複を 1 つ減らして
保守対象を数百行増やす取引になる。テストを足して整備するのは、存廃が決まった後。

この門は D-3（`legacy/MapResource`）と B/C（`Material` 系）の両方に効く。
**同じ事実には同じ結論を出す。**

### 判定 2 — 差が観測できないなら、消す

実際に渡されうるどんな入力を与えても違いが出ないなら、それは重複。1 本へ寄せる。
...
黙って終わらせない。**`docs/backlog.md` へトリガー付きで起票する。**
...
| 場所 | 本体 |
|---|---|
| `engine/src/core/Num.flix:10` | **pub。寄せ先** |
...
- `ShaderEval.flix:427` の `lerp` は `Num.lerp`(:35) と一致 → 寄せる
- `fract` は `Num.fract`(:19) のほかに **5 本**（`ShaderEval.flix:423` / `Curve.flix:10` /
  `SfxSynth.flix:23` / bench 2 本）。engine / engine_world / engine_tools の 3 本を寄せる。
  **bench の 2 本は寄せない** — bench はエンジンの外から呼ぶ計測台で、寄せると計測対象に
  エンジンの都合が混ざる（判定 3 の 1 番: 別パッケージであることが差を言っている）
- `ShaderDoc.smoothstepF`(:367) と `ShaderEval.smoothstep`(:432) は clamp01 のインライン展開以外
  バイト一致。ただし **`Num.smoothstep` は存在しない**。寄せるには engine に新しい pub を
  生やすことになり、これは重複解消ではなく **engine API の拡張**（AGENTS.md の事前相談の対象）。
  → **この回では寄せない。**「人へ聞くこと」に追加する（下の工程 0）
...
「差が無い」ではない。→ **寄せない。判定 3 の 2 番（名前）で差を言う**
...
**裁定 G（門 0）— この回では寄せない。起票のみ。**

理由 2 つ。(a) `RadialGlow` の `normalizedRadius` / `smoothstepCurve` / `haloPixel` / `maskPixel` は
**pub**。(b) 寄せ先になる engine 側の `argb` / `toByte` は **private** なので、寄せるには
...
現状を正当化するコメントがあり、これも寄せるなら同時に直す対象になる。
...
単位の定義（差を引数に出せる部分を切り出す）が効く唯一の裁定。**
...
依存の向きの言い訳が効かない唯一の組で、前の版はここを裁いていなかった。
...
残す理由は、**fail-open する口と fail-closed な口の両方が実際に要る**から。差は戻り型で言えている
...
**各段の前に `make sync`。** `sync-engine` → `sync-render-gl` → `sync-engine-world` →
...
寄せるには engine に新しい pub が要る（門 0 + engine 拡張）。**基準から導くと降格**なので
...
- **共通の `Mix` モジュールを新設する** — 寄せ先の `Color.mix` が既にある。新設は 7 本目を作るだけ
