...
リリース前の安全策(同日、人の指示で確定):

8. **DaemonViable**: repl 1 本の予算 (heapMinMB + headroom) が物理メモリ × memoryShare に
   収まらない機械では常駐せず、毎回素の CLI で検査する(8GB 機で 3.6GB の JVM を立てない)。
   クライアントの入口と起動予約 (tooBusy) の両方で見る。環境変数 CHECKD_RSS_MB を明示した
   人は常駐してよい側に倒す。
9. **warmTimeoutSec のフォールバック既定値**: キーが無い古い hooks.json でも既定値 240 で起動する
   (バイナリだけ部分更新した clone を壊さない)。1 リリースの間だけ。次のリリースで
   必須キーへ昇格させ、既定値を消す(LoadRules 内のコメント参照)。
- 常駐を完全に無くす案は不採用(check 0.4 秒 ⇔ 素の check 1〜2 分の差が大きく、
  ハングの原因は常駐という作りでなく設定だったため)。3 プロジェクト同時のための
  LRU 追い出しも見送り(段 4 のメモとしてだけ残す)。
